...
%   equation
% - dTx and dTy contain the solution with respect to x and y respectively

%% Application of the finite differences method
%  Coefficients corresponding to the approximation of the first derivative
% (of 4th order), for the Fokker-Planck equation solution
...
s2=A2\b2;
cf2=sum(s2);
% Matrix A will contain the coefficients of the system corresponding to the
% trivial solution of the stationary solution of the Fokker-Planck equation
% For this approximation it was used dx=dy 
%% Computation of values of the invariant density
A=zeros(length(dox)*length(doy));                                          % preallocated for efficiency                                      
% Coefficient for (x(i),y(j))                                           
for i=1:length(dox)
    for j=1:length(doy)
   A((i-1)*length(doy)+j,(i-1)*length(doy)+j)=-(P(3)*cf2/(dy^2))+(cf1*doy(j)/dy)+(P(1)*dox(i)^2-P(2))-(cf1*((P(1)*dox(i)^2-P(2))*doy(j)+P(6)*(dox(i)^3+P(4)*dox(i)+P(5)))/dy); %%Mod 
    end
end
% Coefficient for(x(i),y(j+1))
for i=1:length(dox)
    for j=1:length(doy)-1
   A((i-1)*length(doy)+j,(i-1)*length(doy)+j+1)=(P(3)*s2(3)/(dy^2))+(s1(2)*((P(1)*dox(i)^2-P(2))*doy(j)+ P(6)*(dox(i)^3+P(4)*dox(i)+P(5)))/dy); %%Mod
    end
end
% Coefficient for(x(i),y(j+2))
for i=1:length(dox)
    for j=1:length(doy)-2
   A((i-1)*length(doy)+j,(i-1)*length(doy)+j+2)=(P(3)*s2(2)/(dy^2))+(s1(1)*((P(1)*dox(i)^2-P(2))*doy(j)+ P(6)*(dox(i)^3+P(4)*dox(i)+P(5)))/dy); %%Mod
    end
end
% Coefficient for(x(i),y(j+3))
for i=1:length(dox)
    for j=1:length(doy)-3
   A((i-1)*length(doy)+j,(i-1)*length(doy)+j+3)=(P(3)*s2(1)/(dy^2));
    end
end
% Coefficient for(x(i),y(j-1))
for i=1:length(dox)
    for j=2:length(doy)
   A((i-1)*length(doy)+j,(i-1)*length(doy)+j-1)=(P(3)*s2(4)/(dy^2))+(s1(3)*((P(1)*dox(i)^2-P(2))*doy(j)+ P(6)*(dox(i)^3+P(4)*dox(i)+P(5)))/dy); %%Mod
    end
end
% Coefficient for(x(i),y(j-2))
for i=1:length(dox)
    for j=3:length(doy)
   A((i-1)*length(doy)+j,(i-1)*length(doy)+j-2)=(P(3)*s2(5)/(dy^2))+(s1(4)*((P(1)*dox(i)^2-P(2))*doy(j)+P(6)*(dox(i)^3+P(4)*dox(i)+P(5)))/dy); %%Mod
    end
end
% Coefficient for(x(i),y(j-3))
for i=1:length(dox)
    for j=4:length(doy)
   A((i-1)*length(doy)+j,(i-1)*length(doy)+j-3)=(P(3)*s2(6)/(dy^2));
    end
end
...
Snoc=[Sno(1:(od-1));1;Sno((od):end)];                                      % solution
